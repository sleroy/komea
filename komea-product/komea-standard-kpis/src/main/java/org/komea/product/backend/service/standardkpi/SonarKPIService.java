...
public class SonarKPIService {
    ...
    public Kpi buildSonarMetricKpi(final String _name, final String _key, final double _min, final double _max, final ValueType _valueType,
        ...
        return KpiBuilder.createAscending().nameAndKeyDescription(_name).entityType(EntityType.PROJECT).providerType(ProviderType.QUALITY)
                .interval(_min, _max).produceValue(_valueType, _valueDirection).key(_key).queryScript(groovyScriptLoader.load()).build();
    }
    ...
    public void initSonarKPI() {
        ...
        saveOrUpdate(buildSonarMetricKpi("Public documented API (%)", "public_documented_api_density", 0, 100, ValueType.PERCENT,
        ...
        saveOrUpdate(buildSonarMetricKpi("Branch coverage on new code", "new_branch_coverage", 0, 100, ValueType.PERCENT,
                ValueDirection.BETTER));
        saveOrUpdate(buildSonarMetricKpi("Coverage on new code", "new_coverage", 0, 100, ValueType.PERCENT, ValueDirection.BETTER));
        saveOrUpdate(buildSonarMetricKpi("Duplicated lines (%)", "duplicated_lines_density", 0, 100, ValueType.PERCENT,
