...
public class SonarKPIService {
	...
	public Kpi buildSonarMetricKpi(final String _title, final String _metricName, final double _min, final double _max) {

		final GroovyScriptLoader groovyScriptLoader = new GroovyScriptLoader(Thread.currentThread()
		        .getContextClassLoader(), "org/komea/product/backend/service/standardkpi/SonarMetricScript.groovy");
		groovyScriptLoader.addParameter("##metric##", _metricName);

		return KpiBuilder.createAscending().nameAndKeyDescription(_title).dailyKPI().entityType(EntityType.PROJECT)
		        .providerType(ProviderType.QUALITY).interval(_min, _max).queryScript(groovyScriptLoader.load()).build();
	}
	...
	public void initSonarKPI() {
		...
		saveOrUpdate(buildSonarMetricKpi("Public documented API (%)", "public_documented_api_density", 0, 100));
		...
		saveOrUpdate(buildSonarMetricKpi("Branch coverage on new code", "new_branch_coverage", 0, 100));
