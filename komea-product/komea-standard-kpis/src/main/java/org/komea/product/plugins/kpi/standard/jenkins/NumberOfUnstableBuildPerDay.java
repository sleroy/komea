 ...
 * @author sleroy
 */
public class NumberOfUnstableBuildPerDay extends AbstractCEPQueryImplementation {

	/**
     * 
     */
	public NumberOfUnstableBuildPerDay() {
	...
	public List<IFilterDefinition> getFilterDefinitions() {
		...
		final IEventFilter<?> eventFilter = EventFilterBuilder.create().chain(new OnlyEventFilter())
		        .chain(new WithProjectFilter()).chain(new EventTypeFilter("build_unstable")).build();
