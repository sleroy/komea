 ...
 * @author sleroy
 */
public class NumberOfFixedBuildPerUserPerDay extends AbstractCEPQueryImplementation {
	...
	public List<IFilterDefinition> getFilterDefinitions() {
		...
		final IEventFilter<?> eventFilter = EventFilterBuilder.create().chain(new OnlyEventFilter())
		        .chain(new WithUserFilter()).chain(new EventTypeFilter("build_fixed")).build();
