 ...
 * @author sleroy
 */
public class NumberOfSuccessfulBuildPerDay extends AbstractCEPQueryImplementation {

	/**
     * 
     */
	public NumberOfSuccessfulBuildPerDay() {
	...
	public List<IFilterDefinition> getFilterDefinitions() {
		...
		final IEventFilter<?> eventFilter = EventFilterBuilder.create().chain(new OnlyEventFilter())
		        .chain(new WithProjectFilter()).chain(new EventTypeFilter("build_complete")).build();
