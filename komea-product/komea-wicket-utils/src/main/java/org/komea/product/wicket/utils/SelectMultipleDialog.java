...
public abstract class SelectMultipleDialog extends AbstractFormDialog<String> {
    ...
    public SelectMultipleDialog(
        ...
        listEntite = new ListMultipleChoice<IHasKey>("tableMult", new PropertyModel<List<IHasKey>>(this,
    ...
    private void defineSizeList() {
        if (listUsed.size() < 8) {
            listEntite.setMaxRows(8);
        } else {
            listEntite.setMaxRows(listUsed.size() + 1);
        }
    }
