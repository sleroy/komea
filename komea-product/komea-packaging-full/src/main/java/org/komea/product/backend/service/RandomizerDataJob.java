...
public class RandomizerDataJob implements Job {
	...
	private void generateKpiValues(final Kpi _kpi, final IEntity _entity) {
		...
		while (previousTime.isBefore(dateTime)) {
			Double lastStoredValueInHistory = statisticsAPI.getLastStoredValueInHistory(HistoryKey.of(_kpi, _entity));
			...
			statisticsAPI.storeValueInHistory(HistoryKey.of(_kpi, _entity), lastStoredValueInHistory, previousTime);
