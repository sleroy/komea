...
public class UserCommitFormula implements ICEPFormula<IScmCommit, KpiResult> {
	...
	public KpiResult compute(final ICEPStatement<IScmCommit> _arg0) {
		...
		final ScmCommitTable<EntityKey> scmCommitPerDayTable = ScmCommitTable.buildTableFromCommitsAndKey(
		        aggregateView, new ScmUserGroupingFunction());

		for (final EntityKey userKey : scmCommitPerDayTable.keys()) {
			final Collection<IScmCommit> userCommits = scmCommitPerDayTable.getListOfCommitsPerKey(userKey);
