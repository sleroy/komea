...
public class BZConfigurationDAO implements IBZConfigurationDAO
{
    ...
    public boolean delete(final BZServerConfiguration _object) {
        ...
        final boolean delete = configurationStorage.delete(_object);
    ...
    public boolean testConnexion(final BZServerConfiguration server)
            throws IOException, ConnectionException, BugzillaException {
    
    
        IBZServerProxy newConnector = null;
        boolean connexion = false;
        try {
            newConnector = serverProxyFactory.newTestConnector(server);
            ...
            IOUtils.closeQuietly(newConnector);
        }
        return connexion;
    }
}
