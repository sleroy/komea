...
public class BZProviderPlugin {
    ...
    public Kpi bzClosedBugs() {
        ...
        return bzKpi("bugs_status_closed", "Closed bugs", "Number of closed bugs", ValueDirection.BETTER, new GroovyScriptLoader(
                "scripts/BzClosedBugScript.groovy").load());
    }
    
    public Kpi bzOpenBugs() {
    
        return bzKpi("bugs_status_open", "Open bugs", "Number of open bugs", ValueDirection.WORST, new GroovyScriptLoader(
                "scripts/BzOpenBugScript.groovy").load());
    }
    
    public Kpi bzOpenBySeverityBugs(final String severity) {
    
        final GroovyScriptLoader groovyScriptLoader = new GroovyScriptLoader("scripts/BzOpenBySeverityBugs.groovy");
        ...
        return bzKpi("bugs_status_open_severity_" + severity, "Open " + severity + " bugs", "Number of open bugs with " + severity
                + " severity", ValueDirection.WORST, groovyScriptLoader.load());
    }
    ...
    public Kpi bzOpenNotFixedBugs() {
        ...
        return bzKpi("bugs_status_open_not_fixed", "Open NOT fixed bugs", "Number of open but not fixed bugs", ValueDirection.WORST,
                new GroovyScriptLoader("scripts/BzOpenNotFixedBugs.groovy").load());
    }
    
    public Kpi bzTotalBugs() {
    
        return bzKpi("bugs_total", "Total bugs", "Total number of bugs", ValueDirection.WORST, new GroovyScriptLoader(
                "scripts/BzTotalBugs.groovy").load());
    }
    ...
    private Kpi bzKpi(final String key, final String name, final String description, final ValueDirection valueDirection,
        ...
        return KpiBuilder.create().name(name).key(key).description(description).interval(0d, 1000d).providerType(ProviderType.BUGTRACKER)
                .queryScript(formula).forProject().produceValue(ValueType.INT, valueDirection).build();
