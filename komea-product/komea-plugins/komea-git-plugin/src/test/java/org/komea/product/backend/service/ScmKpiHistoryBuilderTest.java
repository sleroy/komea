...
public class ScmKpiHistoryBuilderTest extends AbstractSpringIntegrationTestCase {
    ...
    private final class DayGroupingFunction implements IScmCommitGroupingFunction<ITuple> {
        ...
        public ITuple getKey(final IScmCommit _commit) {
            ...
            return TupleFactory.newTuple(commitTime.year().get(), commitTime.monthOfYear().get(), commitTime.dayOfMonth().get());
        }
    }
    ...
    public void testKpiHistoryBUilder() {
        ...
        // for (final ScmRepositoryDefinition scmRepositoryDefinition : repositoryService.getAllRepositories()) {
    ...
    // .withMonthOfYear((Integer) periodDate.values().get(1)).withDayOfMonth((Integer) periodDate.values().get(2));
    // return analysisDate;
    // }
    //
    ...
    // LOGGER.info("SCM Repository {} has {} periods to backup in history", scmRepositoryDefinition.getRepoName(),
    ...
    // }
    //
    // final List<Measure> filteredHistory = measureHistoryService.getHistory(HistoryKey.of(scmKpiPlugin.numberOfCommitsPerDayPerUser()
    // .getId()));
    ...
    // }
    //
    // private void buildUserStatistics(final DateTime analysisDate, final ScmCommitTable<ITuple> userTable, final ITuple user) {
    //
    // final Collection<IScmCommit> commitsOfTheDay = userTable.getListOfCommitsPerKey(user);
    // measureHistoryService.storeMeasure(buildHistoryKey(user, scmKpiPlugin.numberOfCommitsPerDayPerUser().getId()), new Double(
    // new NumberOfCommitsPerDay().compute(commitsOfTheDay)), analysisDate);
    // measureHistoryService.storeMeasure(buildHistoryKey(user, scmKpiPlugin.numberOfAddedLinesPerUser().getId()), new Double(
    // new NumberOfAddedLinesPerDay().compute(commitsOfTheDay)), analysisDate);
    // measureHistoryService.storeMeasure(buildHistoryKey(user, scmKpiPlugin.numberofDeletedLinesPerDayPerUser().getId()), new Double(
    // new NumberOfDeletedLinesPerDay().compute(commitsOfTheDay)), analysisDate);
    // measureHistoryService.storeMeasure(buildHistoryKey(user, scmKpiPlugin.numberOfChangedFilesPerDayPerUser().getId()), new Double(
    // new NumberOfModifiedFilesPerDay().compute(commitsOfTheDay)), analysisDate);
    // measureHistoryService.storeMeasure(buildHistoryKey(user, scmKpiPlugin.numberOfChangedLinesPerDayPerUser().getId()), new Double(
    // new NumberOfModifiedLinesPerDay().compute(commitsOfTheDay)), analysisDate);
    // measureHistoryService.storeMeasure(buildHistoryKey(user, scmKpiPlugin.averageCommitMessageLength().getId()), new Double(
    // new AverageCommitMessageLength().compute(commitsOfTheDay)), analysisDate);
    // measureHistoryService.storeMeasure(buildHistoryKey(user, scmKpiPlugin.numberTotalOfModifiedLinesPerUser().getId()), new Double(
