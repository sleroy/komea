 ...
 * @author sleroy
 */
public abstract class AbstractIssuePersonKPI extends
...
AbstractDynamicQuery
{
    ...
    public AbstractIssuePersonKPI(final BackupDelay _delay, final String _dynamicSource) {
    ...
    public KpiResult evaluateResult() {
        ...
        final MultiMap<Person, IIssue> map = new MultiMap<Person, IIssue>();
        ...
        for (final IIssue issue : listOfIssues) {
            final Person handler = obtainPerson(issue);
            if (handler == null) {
                continue;
            }
    ...
    protected abstract Person obtainPerson(final IIssue issue);
