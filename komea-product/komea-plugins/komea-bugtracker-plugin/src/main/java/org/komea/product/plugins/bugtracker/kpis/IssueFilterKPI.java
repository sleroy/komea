...
AbstractDynamicQuery
{
    ...
    public KpiResult evaluateResult() {
        ...
        LOGGER.debug("Obtained {} bugtracking data sources", issuePlugins.length);
        for (final IIssuePlugin plugin : issuePlugins) {
            groupMap(plugin, map);
        }
        
        LOGGER.debug("Applying the filter per handler and obtaining results : {} items", map.size());
        for (final IEntity person : map.keySet()) {
            ...
            // }
            LOGGER.debug("Filter returned {} elements", map.get(person).size());
            kpiResult.put(person, map.get(person).size());
        }
    ...
    private void groupMap(final IIssuePlugin _plugin, final ListMultimap<IEntity, IIssue> _map) {
        ...
        LOGGER.debug("Obtaining results from the dynamic data source");
        final List<IIssue> listOfIssues = _plugin.getData();
        LOGGER.debug("Results obtained : {} elements", listOfIssues.size());
        LOGGER.debug("Sorting results per handler");
