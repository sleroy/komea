...
public class ProjectFormula implements ICEPFormula<IEvent, KpiResult> {
	...
	public KpiResult compute(final ICEPStatement<IEvent> _arg0) {
		...
		final EventTable<EntityKey, IEvent> eventTable = new EventTable<EntityKey, IEvent>(new ProjectTupleCreator());
	...
	private boolean isInvalidEntity(final Entry<EntityKey, IEventGroup<IEvent>> eventGroupEntry) {
		...
		return eventGroupEntry.getKey().isUncompleteKey() || !eventGroupEntry.getKey().isProjectKey();
	}
}
