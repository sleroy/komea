 ...
 * @author sleroy
 */
public class TeamFormula implements ICEPFormula<IEvent, KpiResult>
{
     ...
     */
    public TeamFormula(final IEventGroupFormula<IEvent> _eventValueFormula) {
    ...
    public KpiResult compute(final ICEPStatement<IEvent> _arg0, final Map<String, Object> _arg1) {
        ...
        final EventTable<EntityKey, IEvent> eventTable =
                new EventTable<EntityKey, IEvent>(new TeamTupleCreator());
    ...
    private boolean isInvalidEntity(final Entry<EntityKey, IEventGroup<IEvent>> eventGroupEntry) {
        ...
        return eventGroupEntry.getKey().isUncompleteKey() || !eventGroupEntry.getKey().isTeamKey();
    }
}
