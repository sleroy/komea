...
public class StatisticsService implements IStatisticsAPI {
    ...
    @Override
    public void mergeEntitiesMeasures(final IEntity entitySrc, final IEntity entityDest) {
        Validate.isTrue(entitySrc.entityType().equals(entityDest.entityType()));
        final List<Kpi> kpis = kpiService.getAllKpisOfEntityType(entitySrc.entityType());
        for (final Kpi kpi : kpis) {
            if (entityHasKpiMeasures(entitySrc, kpi) && !entityHasKpiMeasures(entityDest, kpi)) {
                final List<Measure> measures = getMeasures(entitySrc, kpi);
                for (final Measure measure : measures) {
                    measure.setEntityID(entityDest.getId());
    ...
    private MeasureCriteria getCriteriaOfEntityAndKpi(final IEntity entity, final Kpi kpi) {
        final MeasureCriteria criteria = new MeasureCriteria();
        final String idKpi = FormulaID.of(kpi).getId();
        criteria.createCriteria().andIdKpiEqualTo(idKpi).andEntityIDEqualTo(entity.getId());
        LOGGER.info(kpi.getId() + " - " + idKpi + " - " + entity.getId());
        return criteria;
    }

    private List<Measure> getMeasures(final IEntity entity, final Kpi kpi) {
        final MeasureCriteria criteria = getCriteriaOfEntityAndKpi(entity, kpi);
        return measureDao.selectByCriteria(criteria);
    }

    private boolean entityHasKpiMeasures(final IEntity entity, final Kpi kpi) {
        final MeasureCriteria criteria = getCriteriaOfEntityAndKpi(entity, kpi);
        return measureDao.countByCriteria(criteria) > 0;
    }
