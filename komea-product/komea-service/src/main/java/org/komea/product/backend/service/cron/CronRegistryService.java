...
public class CronRegistryService implements ICronRegistryService, ApplicationContextAware {
	...
	public List<CronDetails> getCronTasks() {
		...
		try {
			for (final JobKey jobKey : schedulerFactory.getScheduler().getJobKeys(GroupMatcher.anyJobGroup())) {
	...
	@Override
	public void registerCronTask(final String _cronName, final BackupDelay _backupDelay,
	        final Class<? extends Job> _runnable, final JobDataMap _properties) {
	 ...
	 */
	@Override
	public void registerCronTask(final String _cronName, final String _cronExpression,
	        final Class<? extends Job> _runnable, final JobDataMap _properties) {
	...
	public void setApplicationContext(final ApplicationContext _applicationContext) throws BeansException {
		...
		try {
			...
			schedulerFactory.getScheduler().setJobFactory(new JobInjectionFactory(_applicationContext));
	 ...
	 * @param _cronExpression
	 *            String
	 * @see org.komea.product.backend.service.cron.ICronRegistryService#updateCronFrequency(String,
	 *      String)
	 */
	@Override
	public void updateCronFrequency(final String _cronName, final String _cronExpression) {
		...
		try {
			final JobDetail jobDetail = schedulerFactory.getScheduler().getJobDetail(JobKey.jobKey(_cronName));

			final JobDataMap jobDataMap = jobDetail.getJobDataMap();

			final Trigger trigger = TriggerBuilder.newTrigger().forJob(_cronExpression).usingJobData(jobDataMap)
			        .withIdentity(_cronName).startNow().withSchedule(CronScheduleBuilder.cronSchedule(_cronExpression))
			        .build();
			schedulerFactory.getScheduler().rescheduleJob(TriggerKey.triggerKey(_cronExpression), trigger);
		} catch (final SchedulerException e) {
			throw new CronRuntimeException("Impossible to update the cron frequency of the task : " + _cronName, e);
		}

	}

	private void createNewCronJob(final String _cronName, final Class<? extends Job> _runnable,
	        final JobDataMap _properties, final CronScheduleBuilder cronSchedule) throws CronRuntimeException {
		try {

			LOGGER.info("Registering new CRON {}", _cronName);
			final JobDetail jobDetail = JobBuilder.newJob(_runnable).withIdentity(_cronName).withDescription(_cronName)
			        .usingJobData(_properties).build();

			final Trigger trigger = TriggerBuilder.newTrigger().forJob(jobDetail).usingJobData(_properties)
