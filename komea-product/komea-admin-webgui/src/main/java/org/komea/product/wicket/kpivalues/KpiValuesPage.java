...
public class KpiValuesPage extends StatelessLayoutPage
{
    ...
    private final class KpiValueList extends ListView<IEntity>
    {
        ...
        protected void populateItem(final ListItem<IEntity> _item) {
            ...
            final PeriodTimeSerieOptions options =
                    PeriodTimeSerieOptions.buildOptionsFromTimeScale(kpi, TimeScale.PER_DAY);
            
            _item.add(new Label("lastvalue", Model.of(statsService.evaluateKpiValueOnPeriod(
                    options, historyKey.getEntityKey()))));
            _item.add(new Label("currentvalue", Model.of(statsService
                    .returnsTheLastValue(historyKey))));
        }
    }
    ...
    public KpiValuesPage(final PageParameters _parameters) {
        ...
        if (kpiChoice == null) {
            ...
            add(new Label("details", kpiQueryService.isEventQuery(kpiChoice)
            ...
            add(new KpiValueList("rows", entitiesByEntityType, kpiChoice));
            if (!kpiQueryService.isEventQuery(kpiChoice)) {
                buildEventList(Collections.EMPTY_LIST);
            } else {
