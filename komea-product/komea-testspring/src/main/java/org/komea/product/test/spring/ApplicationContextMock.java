 ...
 * } are implemented so far. Any other
...
public class ApplicationContextMock implements ApplicationContext, Serializable
{
    ...
    public <T> T getBean(final Class<T> requiredType) throws BeansException {
        ...
        final Iterator<T> beans = getBeansOfType(requiredType).values().iterator();
        
        if (beans.hasNext() == false) {
            throw new NoSuchBeanDefinitionException("bean of required type "
                    + requiredType + " not found");
        }
    ...
    public void putBean(final String name, final Object bean) {
        ...
        if (beans.containsKey(name)) {
            throw new IllegalArgumentException("a bean with name ["
                    + name + "] has already been added to the context");
        }
        beans.put(name, bean);
    }
    
    
    private <A extends Annotation> A findAnnotationOnClass(
            final Class<?> cls,
            final Class<A> annotationType) {
        ...
        if (annotation == null) {
            for (final Class<?> intfClass : cls.getInterfaces()) {
