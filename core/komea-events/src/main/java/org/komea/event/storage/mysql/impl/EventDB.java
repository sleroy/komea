...
public class EventDB implements IEventDB {
    ...
    public EventDB(final ConnectionFactory _dataSource, final String _eventType, final SerializerType serializerType) {
    ...
    private void initSerializer(final SerializerType serializerType) {
        switch (serializerType) {
            case KRYO:
                ...
                unserializer = new KryoByteArrayToObjectConverter<>(kryo, FlatEvent.class);
                serializer = new KryoObjectToByteArrayConverter<>(kryo);
                break;
            default:
                final ObjectMapper jackson = initJackson();
                unserializer = new JacksonByteArrayToObjectConverter<>(jackson, FlatEvent.class);
    ...
    private void createTable(final String _eventType) {
        table = TABLE_PREFIX + _eventType.hashCode();
        Handle handle = null;
        try {
            handle = db.open();
            handle.begin();
...
//        LOGGER.error(_message, _e);
    ...
    private class MysqlResultHandlerMapper implements ResultSetMapper<FlatEvent> {
