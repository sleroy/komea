...
public class EventQueryExecutor {
    ...
    public EventQueryResult execute() {
        ...
        for (final String eventType : eventQuery.getFromEventTypes()) {
            ...
            performQueryOnEventType(eventType, resultMapper);
        }
    ...
    private void performQueryOnEventType(final String eventType, final IResultMapper<FlatEvent> resultMapper) {
        ...
        try {
            ...
            while (eventSetIterator.hasNext() && !reachLimit()) {
                ...
                if (match(next)) {
                    addRowIfExist(resultMapper.process(next));
                }
            }
        } finally {
