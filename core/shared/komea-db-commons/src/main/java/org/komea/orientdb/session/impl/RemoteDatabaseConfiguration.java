...
public class RemoteDatabaseConfiguration extends DatabaseConfiguration {
	...
	public RemoteDatabaseConfiguration(final String _url, final String _databaseName) {
		this(_url, DEFAULT_PORT, _databaseName);
	}

	private String buildURL(final String _url, final int _port, final String _databaseName) {
		// remote:<server>:[<port>]/db-name
		return MessageFormat.format("remote:{0}/{2}", _url, _port, _databaseName);
	}
