...
public class OGraphSchemaUpdater {
	 ...
	 * @return
	 */
	public static String etype(final IEntityType type, final IReference reference) {
	...
	private final static Logger	LOGGER	  = LoggerFactory.getLogger("Orient DB schema updater");
	...
	public void update(final IKomeaSchema schema) {
		...
		this.initializeEdgesTypes(schema);
		this.initializeTypes(schema);

		this.graph.setAutoStartTx(tx);
	}

	private void addProperties(final OrientVertexType vtype, final IEntityType type) {
		for (final IReference property : type.getProperties()) {
			// we only need to create properties for attributes (references on
			// primitive types)
			if (this.isAttribute(property)) {
				final OProperty oproperty = this.createAttribute(vtype, type, property);
				oproperty.setMandatory(property.isMandatory());
			}
		}
	}

	private OrientVertexType create(final IEntityType type, final Set<IEntityType> updated) {
		LOGGER.trace("Create a new Orient DB vertex type for {}", type.getName());
		final IEntityType superType = type.getSuperType();
		OrientVertexType vtype;
		if (superType != null) {
			final OrientVertexType superOrientType = this.getOrUpdateType(superType, updated);
			vtype = this.graph.createVertexType(type.getName(), superOrientType);
		} else {
			vtype = this.graph.createVertexType(type.getName());
		}
		this.addProperties(vtype, type);
		updated.add(type);
		return vtype;
	}

	private OProperty createAttribute(final OrientVertexType vtype, final IEntityType type, final IReference property) {

		OProperty oproperty;

		if (property.isMany()) {
			LOGGER.trace("Create {} list for <{}> property of {}", property.getType(), property.getName(),
					type.getName());
			oproperty = vtype.createProperty(property.getName(), OType.EMBEDDEDLIST);
		} else {
			LOGGER.trace("Create {} attribute for <{}> property of {}", property.getType(), property.getName(),
					type.getName());
	...
	private OrientVertexType getOrUpdateType(final IEntityType type, final Set<IEntityType> updated) {
		OrientVertexType vertexType = this.graph.getVertexType(type.getName());
		if (vertexType == null) {
			vertexType = this.create(type, updated);
		} else {
			if (!updated.contains(type)) {
				vertexType = this.update(vertexType, type);
			}
		}
		return vertexType;
	}
	...
	private void initializeEdgesTypes(final IKomeaSchema schema) {
		...
		final String refEdge = ReferenceKind.REFERENCE.name();
		final OrientEdgeType reference = this.graph.getEdgeType(refEdge);
		if (reference == null) {
			this.graph.createEdgeType(refEdge);
		}
		final String contEdge = ReferenceKind.CONTAINMENT.name();
		final OrientEdgeType containment = this.graph.getEdgeType(contEdge);
		if (containment == null) {
			this.graph.createEdgeType(contEdge);
		}
		final String aggEdge = ReferenceKind.AGGREGATION.name();
		final OrientEdgeType aggregation = this.graph.getEdgeType(aggEdge);
		if (aggregation == null) {
			this.graph.createEdgeType(aggEdge);
		}

		for (final IEntityType type : schema.getTypes()) {
			this.initializeReferencesEdgesTypes(type);
	...
	private void initializeReferencesEdgesTypes(final IEntityType type) {
		for (final IReference property : type.getProperties()) {
			if (!this.isAttribute(property)) {
				final String fqn = etype(type, property);
	...
	private void initializeTypes(final IKomeaSchema schema) {
		...
		for (final IEntityType type : schema.getTypes()) {
			this.getOrUpdateType(type, updated);
		}
	}
	...
	private OrientVertexType update(final OrientVertexType vtype, final IEntityType type) {
		LOGGER.trace("Update existing Orient DB vertex type for {}", type.getName());
