...
public class KomeaEntityFiller<T> implements IKomeaEntityFiller<T> {
	...
	public KomeaEntityFiller(final IKomeaEntityFactory _oKomeaModelFactory,
	...
	public IKomeaEntity put(final T _entity) {
		final Map<String, Serializable> beanMap = new PojoToMap()
		.convertPojoInMap(_entity);
		final IKomeaEntity newInstance = this.oKomeaModelFactory
				.create(this.entityType);
		final Iterator<Entry<String, Serializable>> it = beanMap.entrySet()
				.iterator();
		while (it.hasNext()) {
