...
public class EventStorageTest {
    ...
    public void testDb(final Impl dbImpl) throws Exception {
        EventsService eventStorage = EventStorageFactory.get().newEventStorage(dbImpl);
        ...
        Assert.assertEquals(0, eventStorage.countEventsOfType(NEW_COMMIT));
        ...
        Assert.assertEquals(intervalEvents,
                countEvents(eventStorage.loadEventsOfTypeOnPeriod(NEW_COMMIT, dateInterval)));
    ...
    private long countEvents(final Iterator<KomeaEvent> iterator) {
        long cpt = 0;
    ...
    private EventGenerator getEventGenerator(final IEventsService eventStorage) {
        return new EventGenerator(eventStorage, new IEventDefinition() {
            ...
            public void decorateEvent(final DateTime _date,
                ...
                event.put(KomeaEvent.FIELD_EVENT_TYPE, NEW_COMMIT);
                event.put(KomeaEvent.FIELD_PROVIDER, "perforce");
